...
module.exports = {
    ...
    setReportStatus: (id_report, status, solution) => {
        if(status === 0) {
            ...
            return db.query(`update reports set status = 1, solution = N'${solution}' where type = 0 and id_report = ${id_report}`)
        }
        else {
            // donothing
            return ;
    ...
    setJobReportStatus: (id_report, status, solution) => {
        if(status === 0) {
            ...
            return db.query(`update reports set status = 1, solution = N'${solution}' where type = 1 and id_report = ${id_report}`)
        }
        else {
            // donothing
            return ;
