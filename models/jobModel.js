...
module.exports = {
    getJobsList: (queryArr, multipleTags) => {
        ...
        let finalQuery = `
        select j.*, jri.img, jt.id_tag, t.name as tag_name, t.status as tag_status, p.name as province, d.name as district, jtopic.name as topic_name  ${multipleTags.length > 0 ? ', matches.relevance as relevance' : ''}
        from (((jobs as j left join job_related_images as jri on j.id_job = jri.id_job) left join jobs_tags as jt on j.id_job = jt.id_job) left join tags as t on t.id_tag = jt.id_tag), users as u, provinces as p, districts as d, job_topics as jtopic
        ...
        ${queryArr.length > 0 ? ('where ' + query + ' and j.area_province = p.id_province and j.area_district = d.id_district and jtopic.id_jobtopic = j.id_job') : 'where j.area_province = p.id_province and j.area_district = d.id_district and jtopic.id_jobtopic = j.id_job'} ${multipleTags.length > 0 ? ' and matches.id = j.id_job' : ''}
